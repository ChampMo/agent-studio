...
**archive คือค่าเริ่มต้น แต่ลบถาวรได้** (`DELETE /agents/{id}/permanent`) — ปลอดภัย
เพราะ §5.1: mission ที่จบแล้วถือสำเนาของตัวเองว่าใครรัน ลบ agent ทิ้ง replay ยังบอกชื่อ
model และ avatar เดิมได้ถูกต้อง สิ่งที่หายไปคือที่นั่งในทีมกับความจำของ agent ตัวนั้น
ทีมก็ลบถาวรได้ด้วยเหตุผลเดียวกัน — `missions.team_id` ถูกปล่อยให้ชี้ไปยัง id ที่ไม่มีแล้ว
โดยตั้งใจ เพราะการเขียนทับให้เป็น null เพื่อรักษา foreign key ให้สวย คือการแก้ประวัติศาสตร์
...
```
**ข้อยกเว้นเดียว: ลบทั้ง mission ได้ ถ้าผู้ใช้สั่งเอง** — กฎ append-only มีไว้กัน
การ *เขียนทับ* สิ่งที่เกิดขึ้นแล้ว (ไม่มี UPDATE ไม่มีแก้บรรทัดย้อนหลัง เพราะบันทึกที่แก้ได้
ไม่ใช่บันทึก) การลบทั้งรันตามคำสั่งผู้ใช้ไม่ได้เขียนทับอะไร — ต่างกันเหมือนฉีกทั้งหน้าทิ้ง
กับแก้ตัวเลขในหน้านั้น

ดังนั้น: **ทั้ง mission เท่านั้น ไม่มีการลบ event เดี่ยว ๆ** และ mission ที่ยังรันอยู่ลบไม่ได้ —
ต้องหยุดก่อน เพื่อให้จบด้วย `mission.ended` ตามที่ทุก mission ถูกสัญญาไว้ ลบแล้วไฟล์
artifact ไปด้วย
